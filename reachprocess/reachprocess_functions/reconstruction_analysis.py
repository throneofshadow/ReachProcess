...
def process_csv(prediction_csvs, label_csvs, windowlength=9, thres=0.05, interp_missing=True):
    ...
    for i in range(len(names)):
        ...
        mask = np.isnan(cam1_labelled_x) | np.isnan(cam1_labelled_y) | np.isnan(cam2_labelled_x) | np.isnan(
            cam2_labelled_y) | np.isnan(cam3_labelled_x) | np.isnan(cam3_labelled_y)
        if np.sum(~mask) < 10:
        ...
        entry_dict["Camera 1"]["max_x"] = max(np.max(entry_dict["Camera 1"]["labelled_x"]),
                                              np.max(entry_dict["Camera 1"]["predicted_x"]))
        entry_dict["Camera 1"]["max_y"] = max(np.max(entry_dict["Camera 1"]["labelled_y"]),
                                              np.max(entry_dict["Camera 1"]["predicted_y"]))
        entry_dict["Camera 1"]["min_x"] = min(np.min(entry_dict["Camera 1"]["labelled_x"]),
                                              np.min(entry_dict["Camera 1"]["predicted_x"]))
        entry_dict["Camera 1"]["min_y"] = min(np.min(entry_dict["Camera 1"]["labelled_y"]),
                                              np.min(entry_dict["Camera 1"]["predicted_y"]))
        ...
        entry_dict["Camera 2"]["max_x"] = max(np.max(entry_dict["Camera 2"]["labelled_x"]),
                                              np.max(entry_dict["Camera 2"]["predicted_x"]))
        entry_dict["Camera 2"]["max_y"] = max(np.max(entry_dict["Camera 2"]["labelled_y"]),
                                              np.max(entry_dict["Camera 2"]["predicted_y"]))
        entry_dict["Camera 2"]["min_x"] = min(np.min(entry_dict["Camera 2"]["labelled_x"]),
                                              np.min(entry_dict["Camera 2"]["predicted_x"]))
        entry_dict["Camera 2"]["min_y"] = min(np.min(entry_dict["Camera 2"]["labelled_y"]),
                                              np.min(entry_dict["Camera 2"]["predicted_y"]))
        ...
        entry_dict["Camera 3"]["max_x"] = max(np.max(entry_dict["Camera 3"]["labelled_x"]),
                                              np.max(entry_dict["Camera 3"]["predicted_x"]))
        entry_dict["Camera 3"]["max_y"] = max(np.max(entry_dict["Camera 3"]["labelled_y"]),
                                              np.max(entry_dict["Camera 3"]["predicted_y"]))
        entry_dict["Camera 3"]["min_x"] = min(np.min(entry_dict["Camera 3"]["labelled_x"]),
                                              np.min(entry_dict["Camera 3"]["predicted_x"]))
        entry_dict["Camera 3"]["min_y"] = min(np.min(entry_dict["Camera 3"]["labelled_y"]),
                                              np.min(entry_dict["Camera 3"]["predicted_y"]))
    ...
    for name in filtered_names:
        for cam in ["Camera 1", "Camera 2", "Camera 3"]:
            ...
            data_dict[name][cam]["interp_predicted_prob"][mask] = np.interp(np.flatnonzero(mask), np.flatnonzero(~mask),
                                                                            data_dict[name][cam]["predicted_prob"][
                                                                                ~mask])
            data_dict[name][cam]["interp_predicted_x"] = data_dict[name][cam]["predicted_x"].copy()
            data_dict[name][cam]["interp_predicted_x"][mask] = np.interp(np.flatnonzero(mask), np.flatnonzero(~mask),
                                                                         data_dict[name][cam]["predicted_x"][~mask])
            data_dict[name][cam]["interp_predicted_y"] = data_dict[name][cam]["predicted_y"].copy()
            data_dict[name][cam]["interp_predicted_y"][mask] = np.interp(np.flatnonzero(mask), np.flatnonzero(~mask),
                                                                         data_dict[name][cam]["predicted_y"][~mask])

            # Smoothing
            data_dict[name][cam]["filtered_predicted_prob"] = gaussian_filter(
                data_dict[name][cam]["interp_predicted_prob"], sigma=windowlength)
            data_dict[name][cam]["filtered_predicted_x"] = gaussian_filter(data_dict[name][cam]["interp_predicted_x"],
                                                                           sigma=windowlength)
            data_dict[name][cam]["filtered_predicted_y"] = gaussian_filter(data_dict[name][cam]["interp_predicted_y"],
                                                                           sigma=windowlength)

            data_dict[name][cam]["filtered_labelled_x"] = gaussian_filter(data_dict[name][cam]["labelled_x"],
                                                                          sigma=windowlength)
            data_dict[name][cam]["filtered_labelled_y"] = gaussian_filter(data_dict[name][cam]["labelled_y"],
                                                                          sigma=windowlength)
    return data_dict
...
def get_reconstructions(data_dict, dlt_coefs_file):
    ...
    for name in data_dict["filtered_names"]:
        ...
        xyz_labelled = dlt_reconstruct(dlt_coefs, cam_data, weights)
        ...
        reconstructions[name]["xyz_predicted_prob"] = np.nanmean(np.array(
            [data_dict[name]["Camera 1"]["predicted_prob"], data_dict[name]["Camera 2"]["predicted_prob"],
             data_dict[name]["Camera 3"]["predicted_prob"]]), axis=0)
        reconstructions[name]["xyz_interp_predicted_prob"] = np.nanmean(np.array(
            [data_dict[name]["Camera 1"]["interp_predicted_prob"], data_dict[name]["Camera 2"]["interp_predicted_prob"],
             data_dict[name]["Camera 3"]["interp_predicted_prob"]]), axis=0)
        reconstructions[name]["xyz_filtered_predicted_prob"] = np.nanmean(np.array(
            [data_dict[name]["Camera 1"]["filtered_predicted_prob"],
             data_dict[name]["Camera 2"]["filtered_predicted_prob"],
             data_dict[name]["Camera 3"]["filtered_predicted_prob"]]), axis=0)
