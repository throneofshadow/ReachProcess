...
from reachprocess.utils.kinematic_utils import viz_utils as vu
from reachprocess.phenotyping.Trial_Classifier import Trial_Classify as Classifier
...
class ReachCompute:
    def __init__(self, rat, date, session, robot_data, path_to_video, predictions_data, rmse_data):
        self.tug_flag = None
        ...
        self.block_video_path = path_to_video
    ...
    def load_data(self):
        ...
        df = vu.import_robot_data(self.robot_data)
        self.sensors = df.reset_index(drop=True)
        with (open(self.predictions_data, "rb")) as openfile:
    ...
    def extract_sensor_data(self, idxstrt, idxstp, check_lick=True, filter=False):
        ...
        robot_length = self.block_exp_df['x_pot'].values[0].shape[0]
        x_pot = self.block_exp_df['x_pot'].values[0]  # sample rate at 4x camera
        y_pot = self.block_exp_df['y_pot'].values[0]
        z_pot = self.block_exp_df['z_pot'].values[0]
        r, theta, phi, self.x_robot, self.y_robot, self.z_robot = vu.forward_xform_coords(
